...
// ════════════════════════════════════════════════════════════
// ÚNICO PUNTO DE ENTRADA (2026-08-10): antes abría el modal PROPIO de esta
// ficha (#modalLevSelector, 7 pasos). Daniel pidió unificar con el modal
// "Generar OT" de Tickets (#modalGenerarOT, mismo diseño de 7 pasos +
// agenda/calendario) para no mantener dos implementaciones del mismo
// formulario -- ver templates/tickets/_modal_generar_ot.html (partial
// compartido) y static/tickets_ficha.js (dueño real de la lógica del
// modal). Este archivo define `const TID = null;` (ver bloque de scripts
// de mantenciones/ficha.html) para que tickets_ficha.js entre en
// "_TKOT_MODO_CLIENTE" y use CID/DATA en vez de un ticket real.
// REGLA #4.2: mismo resultado para el usuario -- mismos 7 pasos, mismos
// datos capturados, mismo endpoint final (_mant_lev_crear_ot_core) --
// solo cambia qué modal técnicamente se abre por dentro.
// ════════════════════════════════════════════════════════════
async function abrirLevantamientoSelector(tipoPreset, fechaPreset){
  const modalEl = document.getElementById('modalGenerarOT');
  if (!modalEl) { ilusToast('Modal no inicializado', { type:'error' }); return; }

  // Mismo gate que el modal viejo: sin equipos en la ficha no hay nada que
  // levantar (el modal nuevo sabe crear OTs "de descubrimiento" sin
  // equipos preseleccionados, pero ese flujo es para tickets sin ficha --
  // acá, en la ficha del cliente, seguimos exigiendo al menos 1 equipo).
  ...
  // El modal compartido lee estos "presets" en su listener show.bs.modal
  // (static/tickets_ficha.js) cuando detecta modo cliente -- reemplazan el
  // tipo/fecha por defecto (levantamiento/hoy) que usa cuando lo abre un
  // ticket. Se limpian solos ahí apenas se consumen.
  if (typeof _TKOT !== 'undefined' && _TKOT){
    _TKOT.pendingTipoPreset = tipoPreset || null;
    _TKOT.pendingFechaPreset = (fechaPreset && /^\d{4}-\d{2}-\d{2}$/.test(fechaPreset)) ? fechaPreset : null;
  }
...
// 2026-08-10: el alias `const esc = escHtml;` que vivía acá se retiró --
// colisionaba con `function esc(...)` de static/tickets_ficha.js (ahora
// cargado también en esta página, ver TID=null en mantenciones/ficha.html):
// dos scripts declarando el mismo identificador top-level a nivel global
// (uno con `const`, el otro con `function`) es un SyntaxError real que
// tumbaba TODO tickets_ficha.js al cargar esta ficha. Los 3 usos de
// esc(...) que quedaban (ver más abajo, sección "Inteligencia") se
// reemplazaron por escHtml(...) directo -- mismo comportamiento.
  ...
  try {
    ...
    if (!d.completo){
      html += `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:10px 12px;margin-bottom:8px">
        ...
        <ul style="margin:6px 0 0">${(d.faltantes||[]).map(f=>`<li>${escHtml(f)}</li>`).join('')}</ul></div>`;
    } else {
    ...
    html += `<div style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:10px;padding:10px 12px">
      ...
      <div class="small mb-2" style="color:#374151">${escHtml(a.diagnostico)}</div>`;
    if ((a.acciones||[]).length){
      html += `<div class="small fw-bold mb-1">Recomendaciones:</div><ul class="small" style="margin:0">`;
      a.acciones.forEach(ac => { const col = ac.urgencia==='alta'?'#dc2626':(ac.urgencia==='media'?'#b45309':'#6b7280'); html += `<li style="color:${col}">${escHtml(ac.titulo)}</li>`; });
      html += `</ul>`;
    }
    html += `</div>`;
