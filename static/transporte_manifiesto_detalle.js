...
// ═══════════════════════════════════════════════════════════════════
//  "Agregar factura" al manifiesto (hallazgo H14, 2026-07-29)
//  Antes solo se podia agregar una factura al manifiesto desde el Monitor
//  o el Cubicador -- desde la ficha del manifiesto solo se podia QUITAR.
//  Reusa /transporte/api/compromisos (busqueda, vista=pendientes -- solo
//  documentos con saldo y SIN manifiesto) y el endpoint unificado
//  /transporte/api/manifiestos/asignar (ya corregido esta noche: valida
//  duplicados entre manifiestos, aplica el guard de actividad del
//  courier, recalcula totales) -- no se duplica logica de backend.
// ═══════════════════════════════════════════════════════════════════
var _agModal = null;

function abrirAgregarFactura() {
  if (!_agModal) _agModal = new bootstrap.Modal(document.getElementById('agregarFacturaModal'));
  document.getElementById('agBuscarInput').value = '';
  document.getElementById('agResultados').innerHTML =
    '<div class="text-muted small text-center py-3">' +
    '<i class="bi bi-search me-1"></i>Busca por número de documento, cliente o comuna.</div>';
  _agModal.show();
  setTimeout(function(){ document.getElementById('agBuscarInput').focus(); }, 300);
}

async function agBuscar() {
  var q = (document.getElementById('agBuscarInput').value || '').trim();
  var cont = document.getElementById('agResultados');
  if (!q) {
    cont.innerHTML = '<div class="text-muted small text-center py-3">Escribe algo para buscar.</div>';
    return;
  }
  cont.innerHTML = '<div class="text-center py-3"><div class="spinner-border spinner-border-sm"></div> Buscando…</div>';
  try {
    var r = await fetch('/transporte/api/compromisos?vista=pendientes&q=' + encodeURIComponent(q));
    var d = await r.json();
    var rows = (d && d.compromisos) || [];
    if (!rows.length) {
      cont.innerHTML = '<div class="text-muted small text-center py-3">' +
        '<i class="bi bi-inbox me-1"></i>Sin resultados pendientes para «' + _esc(q) + '».' +
        '<div class="mt-1" style="font-size:.75rem">Solo se muestran documentos con saldo que aún no están en ningún manifiesto.</div></div>';
      return;
    }
    cont.innerHTML = rows.slice(0, 30).map(function(c) {
      var doc = _esc((c.tido || '') + ' ' + (c.nudo || ''));
      var costo = c.costo_zz ? '$' + Math.round(c.costo_zz).toLocaleString('es-CL') : '—';
      return '<div class="d-flex justify-content-between align-items-center border-bottom py-2 px-1">' +
        '<div style="min-width:0">' +
          '<div class="fw-bold font-monospace" style="font-size:.85rem">' + doc + '</div>' +
          '<div class="small text-muted text-truncate" style="max-width:380px">' +
            _esc(c.cliente || '—') + ' · ' + _esc(c.comuna || '—') + ' · ' + costo +
          '</div>' +
        '</div>' +
        '<button class="btn btn-sm btn-outline-dark flex-shrink-0" onclick="agAgregar(' + c.id + ', this)">' +
          '<i class="bi bi-plus-lg"></i> Agregar</button>' +
      '</div>';
    }).join('');
  } catch (e) {
    cont.innerHTML = '<div class="text-danger small text-center py-3">Error de conexión al buscar.</div>';
  }
}

async function agAgregar(cid, btn, confirmDup) {
  if (btn) { btn.disabled = true; btn.innerHTML = '<span class="spinner-border spinner-border-sm"></span>'; }
  try {
    var r = await fetch('/transporte/api/manifiestos/asignar', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({
        commitment_ids: [cid], manifest_id: MID,
        confirm_dup: !!confirmDup,
      }),
    });
    var d = await r.json();
    if (r.status === 409 && d.error === 'duplicado') {
      var lista = (d.duplicados || []).map(function(o){ return o.correlativo + ' (' + o.estado + ')'; }).join(', ');
      var ok = await ilusConfirm({
        title: 'Documento ya en otro manifiesto',
        message: d.msg || 'Este documento ya está en otro manifiesto.',
        sub: lista ? ('Ya está en: ' + lista) : '',
        okLabel: 'Agregar de todas formas', cancelLabel: 'Cancelar',
        danger: true,
      });
      if (ok) return agAgregar(cid, btn, true);
      if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-plus-lg"></i> Agregar'; }
      return;
    }
    if (!d.ok) {
    ...
    ilusToast('✓ Factura agregada al manifiesto', { type: 'success' });
    location.reload();
  } catch (e) {
    ilusToast('Error de conexión al agregar', { type: 'error' });
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-plus-lg"></i> Agregar'; }
  }
}
