...
class TestPlanRamoManifestItem(unittest.TestCase):
    ...
    def test_candado_dos_ramos_del_mismo_documento_en_el_mismo_manifiesto(self):
        """Decisión de Daniel (2026-07-31): si el manifiesto DESTINO ya tiene
        un ramo de este documento, no se puede agregar OTRO ramo del mismo
        documento a ese MISMO manifiesto -- sin override."""
        items = [{"ramo": "despacho", "manifest_id": 5}]
        r = self.plan(["despacho", "instalacion"], items, mid=5)
        self.assertEqual(r, {"accion": "conflicto_candado", "ramo": "instalacion",
                              "ramo_existente": "despacho"})

    def test_candado_tambien_aplica_con_ramo_solicitado_explicito(self):
        """El candado no se puede saltar mandando ramo_solicitado -- sigue
        siendo el mismo documento intentando poner 2 ramos en 1 manifiesto."""
        items = [{"ramo": "despacho", "manifest_id": 5}]
        r = self.plan(["despacho", "instalacion"], items, mid=5, ramo_solicitado="instalacion")
        self.assertEqual(r["accion"], "conflicto_candado")
...
class TestWriteSitesRamoMultiple(unittest.TestCase):
    ...
    def test_los_cuatro_sitios_rechazan_el_candado_de_ramos_con_409(self):
        """Decisión de Daniel (2026-07-31): dos ramos del mismo documento en
        el mismo manifiesto se rechaza SIEMPRE, en los 4 write-sites."""
        for nombre in self.FUNCIONES:
            cuerpo = _norm(_cuerpo_funcion(nombre))
            with self.subTest(funcion=nombre):
                self.assertIn("conflicto_candado", cuerpo,
                             f"{nombre} ya no maneja el resultado 'conflicto_candado' del planner.")
                self.assertRegex(
                    cuerpo,
                    r'conflicto_candado.{0,400}?\), 409',
                    f"{nombre} maneja 'conflicto_candado' pero no devuelve 409.",
                )
    ...
    def test_tr_lineas_pendientes_resuelve_ramo_por_linea_no_por_documento_completo(self):
        ...
        self.assertIn("_clasif_from_skus([s])", cuerpo,
                     ...
                     "multi-ramo fresco.")
        self.assertIn("ramo_solicitado_por_cid", _norm(cuerpo))

    def test_tr_lineas_pendientes_rechaza_dos_ramos_del_mismo_documento_en_el_mismo_pedido(self):
        """Si el operador selecciona líneas de 2 ramos distintos de la MISMA
        factura para el MISMO envío (ej. la línea ZZENVIO y la línea
        ZZINSTALACION), eso es exactamente el candado de Daniel -- se
        rechaza ANTES de tocar la base de datos, con 409."""
        cuerpo = _norm(_cuerpo_funcion("tr_lineas_pendientes_enviar_manifiesto"))
        self.assertIn("_conflictos_mismo_pedido", cuerpo)
        self.assertRegex(cuerpo, r'_conflictos_mismo_pedido.{0,400}?\), 409')

    def test_tr_agregar_item_y_tr_asignar_siguen_preservando_confirm_dup(self):
        """El candado NUEVO (conflicto_candado, sin override) no debe haber
        reemplazado el mecanismo LEGACY preexistente (confirm_dup) para el
        caso distinto: mismo ramo en otro manifiesto -- eso sigue siendo una
...
class TestRespuestasConflictoRamo(unittest.TestCase):
    """_tr_resp_conflicto_candado / _tr_resp_conflicto_multi_ramo arman el
    payload 409 legible que ven los 4 write-sites -- se prueban ejecutando
    las funciones reales (son puras: dict in, dict out)."""
    ...
    def setUpClass(cls):
        ...
        cls.candado = staticmethod(
            _extraer_funcion("_tr_resp_conflicto_candado", extras={"_tr_ramo_label": _label_fn}))
        cls.multi = staticmethod(
            _extraer_funcion("_tr_resp_conflicto_multi_ramo", extras={"_tr_ramo_label": _label_fn}))

    def test_conflicto_candado_menciona_ambos_ramos_en_espanol(self):
        plan = {"accion": "conflicto_candado", "ramo": "instalacion", "ramo_existente": "despacho"}
        r = self.candado(plan, "FCV 12345")
        self.assertEqual(r["error"], "ramo_conflicto")
        self.assertIn("despacho", r["msg"])
        self.assertIn("instalación", r["msg"])
        self.assertIn("FCV 12345", r["msg"])
        self.assertEqual(r["ramo_bloqueado"], "instalacion")
        self.assertEqual(r["ramo_existente"], "despacho")

    def test_conflicto_candado_sin_doc_label_no_revienta(self):
        plan = {"accion": "conflicto_candado", "ramo": "retiro", "ramo_existente": "despacho"}
        r = self.candado(plan)
        self.assertIn("Este documento", r["msg"])
